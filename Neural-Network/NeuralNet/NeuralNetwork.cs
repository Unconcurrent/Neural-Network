...
namespace NeuralNetwork
{
    ...
    public class NeuralNetwork
    {
        ...
        {
            return Run(new List<double>(input));
        }
                    ...
                    else
                    {
                        ...
                        for (int np = 0; np < this.Layers[l - 1].Neurons.Count; np++)
                            neuron.Value = neuron.Value +
                                           this.Layers[l - 1].Neurons[np].Value * neuron.Dendrites[np].Weight;
        ...
        {
            return Train(new List<double>(input), new List<double>(output));}

        public bool Train(List<double> input, List<double> output)
        {
            if ((input.Count != this.Layers[0].Neurons.Count) ||
                (output.Count != this.Layers[this.Layers.Count - 1].Neurons.Count)) return false;
        ...
        {
            var save = new KeyValuePair<List<Layer>, double>(Layers, LearningRate);

            BinaryFormatter bf = new BinaryFormatter();
            save.Save(bf, stream, SerializerCompression.GZip);
        }
        ...
        {
            BinaryFormatter bf = new BinaryFormatter();
            var save = (KeyValuePair<List<Layer>, double>)bf.Deserialize(stream);
            Layers = save.Key;
            LearningRate = save.Value;
        }

        public void Load(string fileName)
        {
            var save = Serializer.Load<KeyValuePair<List<Layer>, double>>("neural.net", SerializerCompression.GZip);
            Layers = save.Key;
            LearningRate = save.Value;
        }
